...
const Dashboard1 = () => {
  ...
  const fetchSlotsForDoctor = async (name) => {
    try {
      const response = await fetch(`http://localhost:3001/doctors/${name.toLowerCase().trim()}/slots`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/doctors/updateAppointment`, {
        method: 'PUT',
        headers: {
      ...
      const slotsResponse = await fetch(`http://localhost:3001/doctors/${formData.doctor.toLowerCase().trim()}/slots`);
      ...
      const appointmentResponse = await fetch('http://localhost:3001/appointments/create', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
